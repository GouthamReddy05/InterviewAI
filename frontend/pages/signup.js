    ...
    <div class="min-h-screen flex items-center justify-center relative overflow-hidden" style="background: linear-gradient(135deg, #0f172a 0%, #1e1b4b 100%);">
        ...
        <div class="relative z-10 w-full max-w-md p-8 bg-surface-900/60 backdrop-blur-xl rounded-2xl border border-surface-700/50 shadow-2xl">
            ...
            <form id="signupForm" class="space-y-5">
                <div>
                    ...
                    <input type="text" id="username" required 
                        class="w-full bg-surface-800/80 border border-surface-700 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all">
                </div>
                ...
                <button type="submit" 
                    class="w-full bg-purple-600 hover:bg-purple-500 text-white font-medium py-3 rounded-xl shadow-[0_0_15px_rgba(147,51,234,0.3)] transition-all hover:-translate-y-0.5 mt-4">
                    Sign Up
                </button>
            </form>
            ...
            <div class="mt-8 text-center text-sm text-surface-400">
                ...
                <button onclick="state.step=6;render()" class="text-purple-400 hover:text-purple-300 font-medium transition-colors">Sign in</button>
            </div>
        </div>
    </div>
    `;
        ...
        document.getElementById('signupForm')?.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    state.step = 0; // Go back to landing
                    render();
                } else {
